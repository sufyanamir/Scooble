...
$(document).ready(function() {
    ...
    // Function to fetch announcements with start_date as today
    function fetchAnnouncements() {
    const today = getCurrentDate();
    var apiname = 'announcements';
    var apiurl = "{{ end_url('') }}" + apiname;  
    var bearerToken = "{{session('user')}}";
    $.ajax({
        url: apiurl + `?current_date=${today}`,
        type: 'GET',
        data: {
        current_date: today,
        },
        ...
        if (response.status === 'success') {
            ...
            // Iterate through the API response and push each announcement into the announcements array
            response.data.forEach((announcementData) => {
            const announcement = {
            ...
            // Now you have the announcements array ready to be used
    ...
    // Call the function to fetch announcements when needed
    fetchAnnouncements();

    // Function to show the announcement modal
    function showAnnouncementModal(announcement) {
        const modalId = `#${announcement.type.toLowerCase()}{{$user->id}}modal_${announcement.id}`;

        // Check if the announcement modal is already shown and dismissed
        const previousDismissed = localStorage.getItem(`dismissed_${modalId}`);
        if (previousDismissed === "true") {
            return;
        }

        // Create the modal dynamically based on the announcement type
        let modalHtml = '';
        switch (announcement.type) {
            case 'Promotion':
                modalHtml = getPromotionModalHtml(announcement);
                break;
            case 'News':
                modalHtml = getNewsModalHtml(announcement);
                break;
            case 'Mandatory':
                modalHtml = getMandatoryModalHtml(announcement);
                break;
            case 'Maintenance':
                modalHtml = getMaintenanceModalHtml(announcement);
                break;
            case 'Warning':
                modalHtml = getWarningModalHtml(announcement);
                break;
            default:
                // Handle unsupported announcement types
                return;
        }
        ...
        // Handle the dismissal of the modal
        $(`${modalId} .dismiss-btn`).on('click', function () {
            $(modalId).modal('hide');
            markAnnouncementAsDismissed(modalId);
        });
    }

    // Function to mark the announcement as dismissed
    function markAnnouncementAsDismissed(modalId) {
        localStorage.setItem(`dismissed_${modalId}`, "true");
    }

    // Function to get Promotion modal HTML
    function getPromotionModalHtml(announcement) {
        return `<div class="modal fade" id="promotion{{$user->id}}modal_${announcement.id}" tabindex="-1" role="dialog" aria-labelledby="promotionmodalLabel_${announcement.id}" aria-hidden="true">
                ...
                <div class="modal-content bg-white p-5">
                    <div class="modal-body mt-5">
                        <div class="alert-div text-center mt-5">
                            ...
                            <button class="btn dismiss-btn dismiss-btn_pro px-5 mt-5" data-dismiss="modal">Dismiss</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // Function to get News modal HTML
    function getNewsModalHtml(announcement) {
        return `<div class="modal fade" id="news{{$user->id}}modal_${announcement.id}" tabindex="-1" role="dialog" aria-labelledby="label_${announcement.id}" aria-hidden="true">
            <div class="modal-dialog mt-5 pt-5" role="document">
                <div class="modal-content bg-white p-5">
                    <div class="modal-body mt-5">
                        <div class="alert-div text-center mt-5">
    ...
    // Function to get Mandatory modal HTML
    function getMandatoryModalHtml(announcement) {
        return `<div class="modal fade" id="mandatory{{$user->id}}modal_${announcement.id}" tabindex="-1" role="dialog" aria-labelledby="label_${announcement.id}" aria-hidden="true">
            <div class="modal-dialog mt-5 pt-5" role="document">
                <div class="modal-content bg-white p-5">
                    <div class="modal-body mt-5">
                        <div class="alert-div text-center mt-5">
    ...
    // Function to get maintenance modal HTML
    function getMaintenanceModalHtml(announcement) {
        return `<div class="modal fade" id="maintenance{{$user->id}}modal_${announcement.id}" tabindex="-1" role="dialog" aria-labelledby="label_${announcement.id}" aria-hidden="true">
            <div class="modal-dialog mt-5 pt-5" role="document">
                <div class="modal-content bg-white p-5">
                    <div class="modal-body mt-5">
                        <div class="alert-div text-center mt-5">
    ...
    // Function to get Warning modal HTML
    function getWarningModalHtml(announcement) {
        return `<div class="modal fade" id="warning{{$user->id}}modal_${announcement.id}" tabindex="-1" role="dialog" aria-labelledby="label_${announcement.id}" aria-hidden="true">
            <div class="modal-dialog mt-5 pt-5" role="document">
                <div class="modal-content bg-white p-5">
                    <div class="modal-body mt-5">
                        <div class="alert-div text-center mt-5">
